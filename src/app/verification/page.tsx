// Old version for verifying via uid, ctr, and cmac
...
import { verifyTag, getProductDetails } from "@/lib/actions/verify";
...
export default async function Page(props: {
  ...
  if (hasAllParams) {
    ...
    if ("error" in verification) {
      ...
      // Fetch product details after successful verification
      const productResponse = await getProductDetails(uid);
            ...
            <CardFooter className="border-t bg-muted/20 p-6 flex justify-center">
              <div className="flex flex-col items-center w-full max-w-md space-y-2">
                <p className="text-sm text-muted-foreground text-center">
                  Every verified tag includes cryptographic signatures to ensure
                  authenticity
                </p>
            ...
            <Tabs defaultValue="overview" className="w-full">
              ...
              <TabsContent value="overview" className="p-4 text-center">
                <p className="text-sm text-muted-foreground mx-auto max-w-xl">
                  Our NFC tag verification system enables you to confirm the
                  authenticity of products, documents, or access credentials
                  with a simple scan. Each tag contains unique cryptographic
                  signatures that can't be replicated, providing you with
                  confidence in the verification result.
                </p>
              </TabsContent>
              <TabsContent value="security" className="p-4 text-center">
                <p className="text-sm text-muted-foreground mx-auto max-w-xl">
                  Authlink uses advanced encryption standards (AES) and
                  specialized NFC security features to protect against
                  counterfeiting. Each verification includes a unique counter
                  value that increases with each scan, allowing you to detect
                  unauthorized duplication attempts.
                </p>
              </TabsContent>
              <TabsContent value="tech" className="p-4 text-center">
                <p className="text-sm text-muted-foreground mx-auto max-w-xl">
                  Our system leverages Secure Dynamic Messaging (SDM) technology
                  with cryptographic message authentication codes (CMAC) to
                  verify tag authenticity. This provides enterprise-grade
                  security while maintaining ease of use for consumers.
                </p>
              </TabsContent>
            </Tabs>
          </div>
    ...
    <div className="space-y-6">
      <div className="rounded-lg border border-green-200 bg-green-50 p-6 text-green-800 dark:border-green-800/30 dark:bg-green-950/50 dark:text-green-200">
        <div className="flex flex-col items-center text-center">
          ...
          <p className="mb-4 text-sm">
            This NFC tag has been authenticated with our secure verification
            system
          </p>
        </div>
      </div>
              ...
              <div className="w-full md:w-2/3 space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                      Code: {productDetails.manufacturer_code}
                    </p>
                  </div>
                  <div>
      ...
      {productDetails?.verification_stats && (
        <div className="rounded-lg border bg-card overflow-hidden">
          <div className="bg-muted/50 px-6 py-3 border-b text-center">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div className="text-center">
                <p className="text-2xl font-bold">
                  {productDetails.verification_stats.total_verifications}
                </p>
                <p className="text-sm text-muted-foreground">
                  Total Verifications
                </p>
              ...
              <div className="text-center">
                <p className="text-2xl font-bold">
                  {productDetails.verification_stats.confirmed_verifications}
                </p>
                <p className="text-sm text-muted-foreground">Confirmed</p>
              </div>
              <div className="text-center">
                <p className="text-2xl font-bold">
                  {productDetails.verification_stats.reported_frauds}
                </p>
                <p className="text-sm text-muted-foreground">Fraud Reports</p>
              </div>
              <div className="text-center">
                <p className="text-sm font-medium">
                  {productDetails.verification_stats.last_verification
                    ? new Date(
                        productDetails.verification_stats.last_verification
                      ).toLocaleDateString()
                    : "Never"}
      ...
      <div className="rounded-md bg-green-50 p-4 border border-green-100 dark:bg-green-950/30 dark:border-green-900/30 flex justify-center">
        <div className="flex items-center gap-2">
          ...
          <p className="text-xs text-green-700 dark:text-green-300">
            Verified with secure cryptographic signature at{" "}
